...
function ChangeRow({ label, current, proposed, currentImages, proposedImages }) {
  ...
  return (
    <div className="rounded-xl border border-amber-100 bg-amber-50/50 p-3">
      <p className="text-[10px] font-bold uppercase tracking-wider text-amber-800">{label}</p>
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-100 bg-white px-3 py-2">
          <p className="text-[9px] font-semibold uppercase text-slate-400">Current (live)</p>
          {hasImages ? (
            <div className="mt-2">
        ...
        <div className="rounded-lg border border-amber-200 bg-white px-3 py-2">
          <p className="text-[9px] font-semibold uppercase text-amber-600">Requested</p>
          {hasImages ? (
            <div className="mt-2">
...
export default function ProfileChangeReviewPanel({ restaurant }) {
  const pending = restaurant?.pendingProfile
  if (!pending || restaurant?.profileReviewStatus !== "pending") return null
  ...
  if (visibleRows.length === 0) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
        <p className="text-sm font-bold text-amber-900">Profile change review</p>
        <p className="mt-1 text-xs text-amber-800">
          {restaurant.pendingUpdateReason || pending.pendingUpdateReason || "Profile update"} — review
          documents below.
        </p>
      </div>
    )
    ...
    <div className="space-y-3 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
      <div>
        ...
        <p className="mt-0.5 text-xs text-amber-800">
          Live approved data vs requested changes. Approve to publish; reject to keep current details.
        </p>
      </div>
