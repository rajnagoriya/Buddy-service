...
const isDiningNavItem = (item) => item.id === "reservations"

const filterDiningItems = (items) =>
  ENABLE_DINING ? items : items.filter((item) => !isDiningNavItem(item))

const filterDiningSections = (sections) =>
  sections
    .map((section) => {
      const items = filterDiningItems(section.items)
      if (!items.length) return null

      // Rename "Delivery & dining" when dining is hidden
      if (!ENABLE_DINING && section.key === "delivery-dining") {
        return { ...section, title: "Delivery", items }
      }
...
export const SIDEBAR_NAV_ITEMS = filterDiningItems(SIDEBAR_NAV_ITEMS_ALL)
export const SIDEBAR_SECTIONS = filterDiningSections(SIDEBAR_SECTIONS_ALL)
export const EXPLORE_SECTIONS = filterDiningSections(EXPLORE_SECTIONS_ALL)
